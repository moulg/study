...
local buttonPanel_ui = require "lobby.ui_create.ui_two_button_tips"

CButtonPanel = class("CButtonPanel",function()
...
function CButtonPanel.create()
	-- body
	local layer = CButtonPanel.new()
	if layer ~= nil then
		layer:regEnterExit()
		return layer
	end
...
function CButtonPanel:regEnterExit()
	-- body
...
function CButtonPanel:onEnter()
	WaitMessageHit.closeWaitMessageHit()
	self:setTouchEnabled(true)
end

function CButtonPanel:onExit()
  
end

function CButtonPanel:init_message(msgID, repArr, isShowTime, sureCallBack, cancelCallBack, arg)
	...
	self.buttonPanel_ui.btnNo:onTouch(closeHandler)
