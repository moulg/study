...
function FishGoldManager:play(info)
	...
	if max_num > jc_system_config.max_gold_arrange_way then
		max_num = jc_system_config.max_gold_arrange_way
	end
	...
	for i=1,max_num do
		...
		if obj then
			...
			distance = ( index )*( math.sqrt( math.pow(content_size.width,2) + math.pow(content_size.height,2) ) + jc_system_config.gold_distance )
			local new_spos = self:getObjPos(max_num,i,info.angle,jc_system_config.gold_arrang_way,distance)
	...
	MusicEffectPlayer.getInstance():play(jc_system_config.gold_mus_id)
end
...
function FishGoldManager:getObjPos(max_num,cur_num,angle,arrange_way,dis)
	...
	elseif arrange_way == 2 then--排列一个矩形
		local line_max = jc_system_config.max_gold_arrange_way2_line
