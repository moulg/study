...
local __battery_ui_creater = require "game.jinchanbuyu.ui.ui_jinchanbuyu_battery"
...
function Battery:initUI()
	...
	self.no_money_spr = cc.Sprite:create("game/jinchanbuyu/resource/other/yubibuzhu.png")
...
function Battery:onTouchBegan(e)
	if self.user_info.is_local == true then
        ...
        if (cut_state == 1) and self:isHaveEnoughScore() == true then
			send_jcby_ReqFire({angle = self.cur_angle,})
	    end
...
function Battery:showEnterAnimation(binit)
	...
	if self.user_info.is_local == true then
		...
		local src_cfg1 = sprite_ani_config[jc_system_config.selfbat_enter_ani_id]
		jc_dyanplay_ani(self.ui_lst.root,src_cfg1,{x = 13,y = 164,},self.cur_angle,1,__init_cmp_call)

		local src_cfg2 = sprite_ani_config[jc_system_config.selfbat_hit_ani_id]
		...
		if binit == false then
			local src_cfg1 = sprite_ani_config[jc_system_config.otherbat_enter_ani_id]
...
function Battery:createGoldColumnObj(item)
	...
	-- 	max_val = jc_system_config.gold_column_max_number*gold_unit_number,
	-- }
...
function Battery:runSendBullet(bsid,angle)
	...
	BulletManager.getInstance():sendBullet(info) -- 生成子弹
...
function Battery:updateLockFishMark()
	...
	-- 			ActionEffectPlayer.getInstance():play(self.ui_lst.SpriteSuoding,nil,jc_system_config.lock_fish_mark_act_id)
	-- 		end
	-- 	end 
	-- else
	-- 	self.ui_lst.SpriteSuoding:stopAllActions()
...
function Battery:updateEngerMark()
	...
	-- 	ActionEffectPlayer.getInstance():play(self.ui_lst.SpriteNengliangpao,nil,jc_system_config.energy_connon_act_id)
	-- else
	-- 	self.ui_lst.SpriteNengliangpao:setVisible(false)
	-- end
end
...
function Battery:onAddBarrelIndex()
	...
	send_jcby_ReqSwitchBattery({type= 0,})
end

function Battery:onMinusBarrelIndex()
	--send message to serve
	send_jcby_ReqSwitchBattery({type=1,})
end

--上分
function Battery:onAddScore()
	send_jcby_ReqExchangeChips({})
end

function Battery:onMinusScore()
	send_jcby_ReqExchangeGolds({})
end
