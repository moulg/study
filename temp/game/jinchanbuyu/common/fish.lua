...
function Fish:createFishItem(data,item_lst,buseShadow,busepit)
	...
	for i=1,len do
		...
		if src_cfg.stype == 1 then
			...
			-- 	item.spr:enableShadow(self.cur_shadow_offset,jc_system_config.fish_shadow_alpha,buseShadow)
			-- end
...
function Fish:update(dt)
	if (BackgroundSceneManager.getInstance():getSceneState() == 2)
		...
		dt = dt*jc_system_config.cut_scene_fish_vx
	end
	...
	if light_obj and self.buse_light_mod == true then
		local vec3 = {x = self.cur_pos.x,y = self.cur_pos.y,z = jc_system_config.fish_light_distance,}
...
function Fish:setUseLightModel(buse)
	...
	-- for k,v in pairs(self.spr_alive_lst) do
	-- 	if v.spr then
	-- 		v.spr:enableShadow(self.cur_shadow_offset,jc_system_config.fish_shadow_alpha,self.buse_light_mod)
	-- 	end
	-- end
end
