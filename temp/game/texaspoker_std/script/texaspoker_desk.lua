...
local desk_item_ui = require "game.texaspoker_std.script.ui_create.ui_texaspoker_desk"

local _contentSize = cc.size(360, 295)

CTexasPokerDesk = class("CTexasPokerDesk", function ()
	-- body
...
function CTexasPokerDesk.getContentSize()
	return _contentSize
end

function CTexasPokerDesk.create()
	-- body
	local layer = CTexasPokerDesk.new()
	if layer ~= nil then
		layer:init_ui()
		return layer
	end
...
function CTexasPokerDesk:init_ui()
...
function CTexasPokerDesk:updatePassword(value)
	self.desk_info.hasPwd = value
	self.desk_item_ui_list.imgLock:setVisible(value == 1)
end

function CTexasPokerDesk:updateSeatInfo( seat )
...
function CTexasPokerDesk:setDeskInfo( info )
	-- body
	self.desk_info = info
...
function CTexasPokerDesk:onEnterDeskClick( e )
	-- body
