...
--找出最大牌型
function texas_card_check.findOutMaxPowerCards(list)
	local cards = texas_card_check.findOutRoyalFlush(list)
	if cards then
		return cards, "皇家同花顺"
	end

	local cards = texas_card_check.findOutFlush(list)
	if cards then
		return cards, "同花顺"
	end

	local cards = texas_card_check.findOutSiTiao(list)
	if cards then
		return cards, "四条"
	end

	local cards = texas_card_check.findOutHuLu(list)
	if cards then
		return cards, "葫芦"
	end

	local cards = texas_card_check.findOutSameFlowers(list)
	if cards then
		return cards, "同花"
	end

	local cards = texas_card_check.findOutShunZi(list)
	if cards then
		return cards, "顺子"
	end

	local cards = texas_card_check.findOutSanTiao(list)
	if cards then
		return cards, "三条"
	end

	local cards = texas_card_check.findOutLiangDui(list)
	if cards then
		return cards, "两对"
	end

	local cards = texas_card_check.findOutDanDui(list)
	if cards then
		return cards, "一对"
	end

	local cards = texas_card_check.findOutSingleCards(list)
	if cards then
		return cards, "单牌"
	end
...
local function checkIsSameFlowers(beginI, endI, tmpCards, powerList)
	...
	for i=beginI,endI do
		for k,id in ipairs(tmpCards[powerList[i]]) do
			local cardType = texas_card_data[id].type
...
--找出皇家同花顺
function texas_card_check.findOutRoyalFlush(list)
	--[[
	tmpCards = {
		[power1] = {id1, id2,},
		[power2] = {id1, },
	...
	for i,v in ipairs(list) do
		local cardData = texas_card_data[v]
...
--找出同花顺
function texas_card_check.findOutFlush(list)
	--[[
	tmpCards = {
		[type] = {id1, id2,},
		[type] = {id1, },
	...
	local function sortCards(id1, id2)
		if texas_card_data[id1].power > texas_card_data[id2].power then
			return true
		end
	...
	for k,cards in pairs(tmpCards) do
		if #cards >= 5 then
			...
			--检测顺子
			return texas_card_check.findOutShunZi(cards)
...
--找出顺子
function texas_card_check.findOutShunZi(list)
	--[[
	tmpCards = {
		[power1] = {id1, id2,},
		[power2] = {id1, },
...
--找出四条
function texas_card_check.findOutSiTiao( list )
	--[[
	tmpCards = {
		[power1] = {id1, id2,},
		[power2] = {id1, },
...
--找出葫芦
function texas_card_check.findOutHuLu( list )
	--[[
	tmpCards = {
		[power1] = {id1, id2,},
		[power2] = {id1, },
	...
	for i,v in pairs(tmpCards) do
		if #v == 3 then
			if arr1 then --存在2对  选择最大的三条
				if texas_card_data[arr1[1]].power < texas_card_data[v[1]].power then
					arr1 =  v	
				end
			else
				arr1 =  v
	...
	for i,v in pairs(tmpCards) do
		...
		if #v == 3 or #v == 2 then
			--是否已经被三条选去
			if texas_card_data[v[1]].power ~= texas_card_data[arr1[1]].power then
				if arr2 then --存在2对  选择最大的一对
					if texas_card_data[arr2[1]].power < texas_card_data[v[1]].power then
						arr2 =  v	
					end
				else
					arr2 =  v
...
--找出同花
function texas_card_check.findOutSameFlowers(list)
	--[[
	tmpCards = {
		[type] = {id1, id2,},
		[type] = {id1, },
...
--找出三条
function texas_card_check.findOutSanTiao( list )
	--[[
	tmpCards = {
		[power1] = {id1, id2,},
		[power2] = {id1, },
...
--找两对
function texas_card_check.findOutLiangDui( list )
	--[[
	tmpCards = {
		[power1] = {id1, id2,},
		[power2] = {id1, },
...
--找一对
function texas_card_check.findOutDanDui( list )
	--[[
	tmpCards = {
		[power1] = {id1, id2,},
		[power2] = {id1, },
...
--找出最大5单牌
function texas_card_check.findOutSingleCards(list)
		--[[
	tmpCards = {
		[power1] = {id1, id2,},
		[power2] = {id1, },
