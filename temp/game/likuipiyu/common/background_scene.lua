...
function BackgroundScene:init(bk_state,cut_t,info)
	...
	local pos = {x = lk_system_config.scene_size.width + pic_size.width/2,y = lk_system_config.scene_size.height/2}

	if bk_state == 0 then
		pos.x = lk_system_config.scene_size.width/2
		pos.y = lk_system_config.scene_size.height/2
	end
	...
	if bk_state == 1 then
		local mov_obj = cc.MoveTo:create(cut_t,{x = lk_system_config.scene_size.width/2,y = lk_system_config.scene_size.height/2})
