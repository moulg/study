...
local __gold_col_creater = require "game.likuipiyu.ui.ui_likuipiyu_goldZhu"
...
function GoldColumn:initUI()
	...
	local exit_ani = cc.AnimationCache:getInstance():getAnimation(sprite_ani_config[lk_system_config.gold_column_ani_src_id].key)
	if exit_ani == nil then
		local animation = self:createAnimation(sprite_ani_config[lk_system_config.gold_column_ani_src_id],true)
		cc.AnimationCache:getInstance():addAnimation(animation,sprite_ani_config[lk_system_config.gold_column_ani_src_id].key)
	end
end
...
function GoldColumn:resetGoldSpr()
	...
	local frame_name = string.format(sprite_ani_config[lk_system_config.gold_column_ani_src_id].pattern,1)
	self.ui_lst.SpriteGold1:setSpriteFrame(frame_name)

	local ani_key = sprite_ani_config[lk_system_config.gold_column_ani_src_id].key
