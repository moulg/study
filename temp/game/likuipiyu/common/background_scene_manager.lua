...
function BackgroundSceneManager:cutBackgroundScene(id)
	...
	if scene_info then
		...
		local pos = {x = lk_system_config.scene_size.width + self.seawave_size.width/4,y = lk_system_config.scene_size.height/2,}
		...
		if self.cut_scene_state == 1 then
			...
			local need_t = self.bg_com_cfg.cut_t*((lk_system_config.scene_size.width + 2.5*self.seawave_size.width/4)/lk_system_config.scene_size.width)
			self.cut_scene_time = need_t
			local mov_obj = cc.MoveTo:create(need_t,{x = -self.seawave_size.width/2,y = lk_system_config.scene_size.height/2,})
