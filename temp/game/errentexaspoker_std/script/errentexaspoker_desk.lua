...
local desk_item_ui = require "game.errentexaspoker_std.script.ui_create.ui_errentexaspoker_desk"

local _contentSize = cc.size(360, 295)

CErRenTexaspokerDesk = class("CErRenTexaspokerDesk", function ()
	-- body
...
function CErRenTexaspokerDesk.getContentSize()
	return _contentSize
end

function CErRenTexaspokerDesk.create()
	-- body
	local layer = CErRenTexaspokerDesk.new()
	if layer ~= nil then
		layer:init_ui()
		return layer
	end
...
function CErRenTexaspokerDesk:init_ui()
...
function CErRenTexaspokerDesk:updatePassword(value)
	self.desk_info.hasPwd = value
	self.desk_item_ui_list.imgLock:setVisible(value == 1)
end

function CErRenTexaspokerDesk:updateSeatInfo( seat )
...
function CErRenTexaspokerDesk:setDeskInfo( info )
	-- body
	self.desk_info = info
...
function CErRenTexaspokerDesk:onEnterDeskClick( e )
	-- body
