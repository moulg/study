...
local desk_item_ui = require "game.tongbiniuniu_std.script.ui_create.ui_tongbiniuniu_desk"

local _contentSize = cc.size(346, 284)

CTongBiNiuNiuDesk = class("CTongBiNiuNiuDesk", function ()
	-- body
...
function CTongBiNiuNiuDesk.getContentSize()
	return _contentSize
end

function CTongBiNiuNiuDesk.create()
	-- body
	local layer = CTongBiNiuNiuDesk.new()
	if layer ~= nil then
		layer:init_ui()
		return layer
	end
...
function CTongBiNiuNiuDesk:init_ui()
...
function CTongBiNiuNiuDesk:updatePassword(value)
	self.desk_info.hasPwd = value
	self.desk_item_ui_list.imgLock:setVisible(value == 1)
end

function CTongBiNiuNiuDesk:updateSeatInfo( seat )
...
function CTongBiNiuNiuDesk:setDeskInfo( info )
	-- body
	self.desk_info = info
	...
	if self.isPlaying == false then
		self.desk_item_ui_list.imgDesk:setTexture("game/tongbiniuniu_std/resource/image/z1.png")
	else
        for i=0,SEAT_NUM-1 do
            key = "imgRead"..i
			self.desk_item_ui_list[key]:setVisible(false)
		end
		self.desk_item_ui_list.imgDesk:setTexture("game/tongbiniuniu_std/resource/image/z2.png")
	end

end

function CTongBiNiuNiuDesk:onEnterDeskClick( e )
	-- body
