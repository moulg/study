...
CTongBiNiuNiuCardCheck = {}

CTongBiNiuNiuCardCheck.CardsKinds = {
	MEI_NIU = 0, --没牛
	NIU_1 	= 1, --牛1
	NIU_2 	= 2, --牛2
	NIU_3 	= 3, --牛3
...
--检测出最大牛牛的点数
function CTongBiNiuNiuCardCheck.checkMaxNiuNiuPower(list)
	if checkIsWuXiao(list) then	--五小
		return CTongBiNiuNiuCardCheck.CardsKinds.WU_XIAO
	elseif checkIsWuHua(list) then --五花
		return CTongBiNiuNiuCardCheck.CardsKinds.WU_HUA
	elseif checkIsSiZha(list) then --四炸
		return CTongBiNiuNiuCardCheck.CardsKinds.SI_ZHA
	elseif checkIsSiHua(list) then --四花
		return CTongBiNiuNiuCardCheck.CardsKinds.SI_HUA
	else
		return CTongBiNiuNiuCardCheck.getNormalNiuNiuMaxPower(list)
	end
end
...
local function checkIsWuXiao(list)
	...
	for k,id in pairs(list) do
		if tongbiniuniu_card_data[id].power >= 5 then
			return false
		end

		totalPower = totalPower + tongbiniuniu_card_data[id].power
	end
...
local function checkIsWuHua(list)
	for k,id in pairs(list) do
		if tongbiniuniu_card_data[id].power < 11 or tongbiniuniu_card_data[id].power > 13 then  --小于J  大于k 
			return false
		end
	end
...
local function checkIsSiHua(list)
	...
	local haveTen = false
	for k,id in pairs(list) do
		if tongbiniuniu_card_data[id].power >= 11 or tongbiniuniu_card_data[id].power <= 13 then  --大于等于J  小于等于k 
			num = num + 1
		end

		if tongbiniuniu_card_data[id].power == 10 then
			haveTen = true
		end
	end
...
local function checkIsSiZha(list)
	local tmp = {}
	for k,id in pairs(list) do
		if tmp[tongbiniuniu_card_data[id].power] then
			tmp[tongbiniuniu_card_data[id].power] = tmp[tongbiniuniu_card_data[id].power] + 1
		else
			tmp[tongbiniuniu_card_data[id].power] = 1
		end
	end
...
local function getNormalNiuNiuMaxPower(list)
	...
	for i,v in ipairs(list) do
		totalPower = totalPower + tongbiniuniu_card_data[v].power

		if tongbiniuniu_card_data[v].four_niuniu_card_type == four_niuniu_card_type.XIAO_WANG or 
			tongbiniuniu_card_data[v].four_niuniu_card_type == four_niuniu_card_type.DA_WANG then
			kingNum = kingNum + 1
		else
			table.insert(tmpList, tongbiniuniu_card_data[v])
		end
	end
	...
	elseif kingNum == 2 then --有两张王，必是牛牛
		maxPower = CTongBiNiuNiuCardCheck.CardsKinds.NIU_NIU
	end
...
--计算底注  庄家筹码  自己的筹码
function CTongBiNiuNiuCardCheck.calute_bottomCash(bankerChips, myChips, playerTotalChips)
...
--返回牌型排列  特殊牌型和无牛排列相同
function CTongBiNiuNiuCardCheck.getCardsTypeFormation( type )
	if type == CTongBiNiuNiuCardCheck.CardsKinds.MEI_NIU or type == CTongBiNiuNiuCardCheck.CardsKinds.SI_HUA or
	type == CTongBiNiuNiuCardCheck.CardsKinds.SI_ZHA or type == CTongBiNiuNiuCardCheck.CardsKinds.WU_HUA or
	type == CTongBiNiuNiuCardCheck.CardsKinds.WU_XIAO then
...
--检测是否是最优牌型
function CTongBiNiuNiuCardCheck.checkIsBestCardsType(list, selecedlist, bestCardsType)
	--特殊牌型和无牛  无需提示
	if CTongBiNiuNiuCardCheck.getCardsTypeFormation( bestCardsType ) == false then
		return true
	end
	...
	for k,v in pairs(list) do
		...
		if isfind == false then
			power = power + tongbiniuniu_card_data[v].mathpower
            if tongbiniuniu_card_data[v].mathpower == 0 then
                isHaveKind = true
            end
		end
	end
    ...
    if isHaveKind then
        power = CTongBiNiuNiuCardCheck.CardsKinds.NIU_NIU
    end
